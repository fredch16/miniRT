...
/*   Updated: 2024/08/19 21:32:57 by fcharbon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	matrix_set_translation(&longer, -2, 0, -1);
	matrix_multiply_matrix(&bigger, &longer, &better);
	sphere_set_transform(obj, &better);
	// obj->capped = false;
	...
	// obj->material.diffuse = 0.7;
	// obj->material.specular = 0.3;
	// sphere_set_transform(obj, &translate_sp);
	...
	sphere_set_transform(obj2, &endlime);
	...
	// sphere_set_transform(obj3, &scale_sp);
	...
	sphere_set_transform(obj4, &endlime);
	...
	matrix_multiply_matrix(&translate_sp, &rotate_pl, &endlime);
	sphere_set_transform(obj5, &endlime);
	...
	// sphere_set_transform(obj7, &translate_sp);
	...
	// sphere_set_transform(obj4, &out3);
	...
	// matrix_multiply_matrix(&out1, &xrota, &out2);
	// matrix_multiply_matrix(&out2, &scale_sp, &out3);
	// sphere_set_transform(obj5, &out3);
	...
	// sphere_set_transform(obj6, &endlime);
	...
	w.point_light.position = tuple_poi(-5, 15.5, -17);
